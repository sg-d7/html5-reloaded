...
function fillTable(users) {
	...
	for (var k in users) {
		...
		tr.append($('<td />').append(btnTemplate.replace('?', id))).appendTo(tBody)
...
$.fn.modBtn = function (modalId) {
	this.on('click', function () {
		...
		modalWindow.find('input').each(function (key, input) {
			...
			$(input).val(userData[input.name]).off('change').on('change', function () {
				userData[this.name] = this.value;
			});
		});
		modalWindow.data('userData', userData);
		modalWindow.find('.mod-save-btn').off('click') // gomb kattintási esemény kezelő lekapcsolása
		.on('click', function () {
